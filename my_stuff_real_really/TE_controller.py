...
def Dukem_n_Nukem(nearest_asteroids: list, ship_state: Dict,current_frame:int) -> Tuple[int, int]:
    ...
    for j in range(0+current_frame,3*30+current_frame+30):
        for f in range(j,j+30):
            for a in nearest_asteroids:
                asteroid = normalize_ahead(a[0], f)
                if in_ring(asteroid, ship_state):
                    final += 1

        if final/initial_count>1/2:
            return(3,max(f+30-3*30,current_frame))
        elif final/initial_count>1/3:
            return(2,max(f+30-3*30,current_frame))
            #if final/initial_count>1/4:
                #return(2,max(j-3*30,0))
